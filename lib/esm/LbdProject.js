...
class LbdProject {
  ...
  async create(existingPartialProjects = [], options = {}, makePublic = false) {
    ...
    let dcatQ = `INSERT DATA {<${this.accessPoint}> <${_vocabCommonRdf.DCTERMS.creator}> "${this.session.info.webId}" ;
  ...
  async findAllPartialProjects(queryEngine = new _querySparql.QueryEngine()) {
    return await (0, _utils.getQueryResult)(this.accessPoint, _vocabCommonRdf.DCAT.dataset, this.fetch, false, queryEngine);
  }
  /**
