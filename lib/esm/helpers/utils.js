...
async function getQueryResult(source, property, fetch, single = false, queryEngine = new _querySparql.QueryEngine()) {
  const q = `SELECT ?res WHERE {<${source}> <${property}> ?res}`;
  const bindings = await queryEngine.queryBindings(q, {
    sources: [source],
    fetch
  }).then(i => i.toArray());
