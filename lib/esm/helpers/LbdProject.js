...
class LbdProject {
  ...
  async getAllPartialProjects() {
    ...
    const results = await this.queryEngine.query(q, {
      sources: [this.accessPoint],
      fetch: this.fetch
    });
    const {
      data
    } = await this.queryEngine.resultToString(results, 'application/sparql-results+json');
  ...
  async getSingleQueryResult(source, property) {
    ...
    const bindings = await this.queryEngine.query(q, {
      sources: [source],
      fetch: this.fetch
    }).then(i => i.bindings());
